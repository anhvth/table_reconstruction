...
parser.add_argument("--scale_size_target", type=int, default=[
                    8, 64], help="scale images to this size before cropping to 256x256")
...
def gen_conv(batch_input, out_channels, stride):
    ...
    if a.separable_conv:
        return tf.layers.separable_conv2d(batch_input, out_channels,\
            kernel_size=4, strides=(stride, 2), padding="same", \
...
def load_examples():
    ...
    with tf.name_scope("target_images"):
        ...
        # target_images = tf.image.resize_images(target_images, a.scale_size_target, method=tf.image.ResizeMethod.AREA)
...
def create_generator(generator_inputs, generator_outputs_channels):
    # 512x512
    vgg16 = tf.contrib.keras.applications.VGG16(weights='imagenet', include_top=False, input_tensor=generator_inputs)
    ...
    _, h, w, _ = generator_inputs.shape
    outputs = tf.image.resize_images(
                outputs, [128 , 1024], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
